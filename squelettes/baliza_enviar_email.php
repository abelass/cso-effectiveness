...
// prepara logo
		$logo = find_in_path('Email.png');
		$url = generer_url_public('',$arg);
		
// prepara el enlace completo
		$enlace = $javascript_centrar_enviar."<a href=\"javascript:;\"
onclick=\"centrarVentana('$url','Enviar_documento','scrollbars=yes,resizable=yes','580','500','true')\" title=\""._T('enviarmail:enviar_title')."\" style=\"border:none\"><img src=\"$logo\" style=\"border:none\" alt=\""._T('enviarmail:enviar_title')."\" title=\""._T('enviarmail:enviar_title')."\" /></a>";
		return $enlace;
...
function balise_ENVIAR_EMAIL($p) {
	...
	if ($GLOBALS['contexte']['id_article']) { 
	$arg = "'page=enviar_email_articulo&amp;id_article='.".champ_sql('id_article', $p);
	}
	elseif ($GLOBALS['contexte']['id_breve']) {
	$arg = "'page=enviar_email_breve&amp;id_breve='.".champ_sql('id_breve', $p);
	}
	elseif ($GLOBALS['contexte']['id_rubrique']) {
	$arg = "'page=enviar_email_rubrique&amp;id_rubrique='.".champ_sql('id_rubrique', $p);
	}
