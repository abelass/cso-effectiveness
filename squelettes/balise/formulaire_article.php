...
if (!empty($variables['actions']['abandonner'])) {
	...
	$var_url="&id_article=". $variables['champs_pri']['id_article'];
	$url_retour = $variables['champs_aux']['url_site'] . $config['UrlAbandon'].$var_url ;
...
if(!empty($variables['actions']['valider'])) {
	...
	lang_select($lang);
		// construction de la page de retour
		$url_retour = $variables['champs_aux']['url_site'] . $config['UrlValidation']. $variables['champs_pri']['id_article'];
		$message = '<META HTTP-EQUIV="refresh" content="'.$config['TempsAtt'].'; url='.$url_retour.'">' . extraire_multi($config['TextValidation']);
